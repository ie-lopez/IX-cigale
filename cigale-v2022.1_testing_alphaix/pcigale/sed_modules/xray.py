...
class Xray(SedModule):
    ...
    parameter_list = {
        ...
        "alpha_ix": (
            "cigale_list()",
            "Power-law slope connecting L at rest-frame 12 microns and 2-10 keV, "
            "defined as αix = log(L(12um) - log(L(2-10keV)."
            "From Asmus et al. 2015, an assuming a slope=1, alpha_ix=0.3",
            (0, 0.15, 0.3, 0.45, 0.6),
        ),
        "max_dev_alpha_ox": (
            "float()",
    ...
    def _init_code(self):
        ...
        self.alpha_ix = float(self.parameters["alpha_ix"])
        ...
        self.nu_10keV = self.c / lam_10keV
    ...
    def process(self, sed):
        ...
        if "agn.L_12um" not in sed.info:
            sed.add_info("agn.L_12um", 0.0, True, unit="W")
        L_12um = sed.info["agn.L_12um"]
        ...
        sed.add_info("xray.alpha_ix", self.alpha_ix)
        ...
        # Calculate L_2-10 from L12um
        # log L12um = 0.3 + log L2-10keV from Asmus+15
        l_2to10keV = L_12um / (10**self.alpha_ix)

        # Calculate L2keV because X-ray module is normalize at 2 keV
        alpha=self.gam-1
        L_2keV = (l_2to10keV*(1-alpha)*self.nu_2keV**-alpha)/(self.nu_10keV**(1-alpha)-self.nu_2keV**(1-alpha))
        Lnu_2keV = L_2keV * self.lam_2keV **2 / self.c
        L_lam_2keV = L_2keV * self.nu_2keV ** 2 / self.c
        ...
        l_agn_2to10keV = self.l_agn_2to10keV * scl_fac                  #Can be optimize because is l_2to10keV
